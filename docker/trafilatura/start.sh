...
# Trafilatura Scraper 启动脚本
# 统一的结果处理：/results/trafilatura/articles/ 和 /results/trafilatura/crawling_report.json

echo "🔍 Starting Trafilatura Scraper..."
echo "Working directory: $(pwd)"

# 设置结果目录
RESULTS_DIR="/results/trafilatura"
...
if python3 trafilatura_scraper.py; then
    echo "✅ Scraping completed successfully!"
    
    # 检查结果是否生成在trafilatura_results目录
    if [ -d "trafilatura_results" ]; then
        echo "📁 Moving results to unified directory structure..."
        
        # 移动文章文件到统一的articles目录
        if [ -d "trafilatura_results/articles" ]; then
            cp -r trafilatura_results/articles/* "$ARTICLES_DIR/" 2>/dev/null || true
        fi
        
        # 复制爬取报告
        if [ -f "trafilatura_results/crawling_report.json" ]; then
            cp "trafilatura_results/crawling_report.json" "$RESULTS_DIR/"
            echo "📊 Report saved to: $RESULTS_DIR/crawling_report.json"
        fi
        
        # 复制其他结果文件
        if ls trafilatura_results/*.json 1> /dev/null 2>&1; then
            cp trafilatura_results/*.json "$ARTICLES_DIR/" 2>/dev/null || true
        fi
...
echo ""
echo "🎉 Trafilatura scraper finished! Check results in:"
echo "   📁 $RESULTS_DIR"
