...
# Requests Scraper 启动脚本
# 统一的结果处理：/results/requests/articles/ 和 /results/requests/crawling_report.json

echo "🕷️ Starting Requests Scraper..."
...
if python3 requests_scraper.py; then
    echo "✅ Scraping completed successfully!"
    
    # 检查结果是否生成在requests_results目录
    if [ -d "requests_results" ]; then
        echo "📁 Moving results to unified directory structure..."
        
        # 移动文章文件到统一的articles目录
        if [ -d "requests_results/articles" ]; then
            cp -r requests_results/articles/* "$ARTICLES_DIR/" 2>/dev/null || true
        fi
        
        # 复制爬取报告
        if [ -f "requests_results/crawling_report.json" ]; then
            cp "requests_results/crawling_report.json" "$RESULTS_DIR/"
            echo "📊 Report saved to: $RESULTS_DIR/crawling_report.json"
        fi
        
        # 复制其他结果文件
        if ls requests_results/*.json 1> /dev/null 2>&1; then
            cp requests_results/*.json "$ARTICLES_DIR/" 2>/dev/null || true
        fi
...
echo ""
echo "🎉 Requests scraper finished! Check results in:"
echo "   📁 $RESULTS_DIR"
