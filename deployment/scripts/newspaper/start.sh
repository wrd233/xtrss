...
# Newspaper Scraper 启动脚本 - 适配新结构
# 统一的结果处理：/app/data/output/YYYY-MM-DD_HH-MM-SS/ 目录结构

echo "📰 Starting Newspaper Scraper (New Structure)..."
...
if python3 -m src.scraper_app.main --scraper newspaper --input data/input/webs.txt --output "$RESULTS_DIR"; then
...
echo ""
echo "🎉 Newspaper scraper finished! Check results in:"
echo "   📁 $RESULTS_DIR"
